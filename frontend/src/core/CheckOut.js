...
const CheckOut = ({
    ...
    const getToken = (user, token) => {
        if (user) {
            getBrainTreeClientToken(user.id, token).then((data) => {
                if (data.error) {
                    setData({
                        ...data,
                        error: data.error
    ...
    const buyProducts = () => {
        ...
        let getNonce = data
            ...
            .then(response => {
                ...
                processPayment(user.id, token, paymentData).then((response) => {
                    let createOrderData = {
                        products: products,
                    ...
                    createOrder(user.id, token, createOrderData).then((response) => {
                        emptyCart(() => {
